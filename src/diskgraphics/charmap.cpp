...
void Charmap::in(std::istream* source)
{
   *source >> this->sizex >> this->sizey;

   if(this->data) delete[] this->data;
   this->data = new unsigned char[this->sizex * this->sizey]();
   char c;
   source->get(c); // this is the line break character, we ignore it
...
std::istream& operator>>(std::istream& is, Charmap& cm)
{
   cm.in(&is);
   return is;
}
